...
$glob = glob("/home/h911249072/web-ohotaktiv.online/docs/obmen_files/quantity/*.xml");
...
while (count($assoc[0]['val'][$h_counter]['val'][0]['val']) > $file_c) {
	...
	if ($z == 0) {
		$h_counter++;
		header("refresh: 2; url=/12dev/quantity/reader.php?h_counter=$h_counter&file_c=$file_count&step_time=$load_time&all_time=$all_time");
		break;
	} 
	...
	$fp = fopen("/home/h911249072/web-ohotaktiv.online/docs/obmen_files/quantity/log.txt", "w"); 
	fwrite($fp, $text); 
	fclose($fp);

	if ($h_counter >= $end && $file_count < $all_files) {
	$file_count++;
	header("refresh: 2; url=/12dev/quantity/reader.php?h_counter=0&file_c=$file_count&step_time=$load_time&all_time=$all_time");
	}
	...
	echo "all_time: ";
	echo "<br>";
	echo "<a href='/12dev/quantity/reader.php?h_counter=0&file_c=0&step_time=0&all_time=0'>repeat</a><br>";
